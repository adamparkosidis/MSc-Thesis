...
target_core_m_factor = numpy.array(list([0.875]))

#epsilon_m_factor = numpy.arange(0.1, 0.2, 0.001)    
epsilon_m_factor = numpy.array(list([0.12]))
...
for dummy_var, core_mult_f in enumerate(target_core_m_factor):
    ...
    path_to_dir_for_data = path+'/radiative_env_TIC/'+dir_name

    if os.path.isdir(path_to_dir_for_data) == False:
        #create directory to store the output
        os.mkdir(path_to_dir_for_data)

    triple_set_up_info(triple, view_on_giant, view_on_se_giant, period_orbit_1, period_orbit_2, eccentricity_orbit_2)
    shutil.move('triple_set_up_info.txt', path_to_dir_for_data)
