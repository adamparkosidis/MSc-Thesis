...
def set_up_inner_binary(orbital_period_1, ecc_1):
    ...
    stars[0].x = semimajor_axis*(1-eccentricity)                           # If the orbit is elliptical we need a correction
...
def estimate_escaping_radius(giant_star, binary_sys):
    ...
    '''
    # 'mass ratio' of giant to inner binary
    q = (giant_star.particles.total_mass() / binary_sys.total_mass())
    # Assume ~ circular orbit:
    a = (giant_star.particles.center_of_mass() - binary_sys.center_of_mass()).length()
    q13 = q**(1./3.)
    q23 = q13**2
    return (a*(0.49*q23/(0.6*q23+math.log(1+q13)))).as_quantity_in(units.RSun)
...
def relax_in_isolation(giant_in_sph, sph_code, output_base_name, mult_factor,epsilon):
    ...
    hydrodynamics.parameters.epsilon_squared = smoothing_length**2  # Smoothing parameter for gravity calculations, removes the singularity in the inverse-square force 
    ...
    giant_in_sph.core_particle.radius = smoothing_length
...
def prepare_giant_system(sph_code, giant_model, view_on_giant, time_unit, n_steps):
    ...
    # Smoothing parameter for gravity calculations, removes the singularity in the inverse-square force. It contains the smoothing length defined in relax_in_isolation 
...
def evolve_coupled_system(binary_system, giant_system, giant_model, inner_binary, computational_domain_removed_particles, \
    ...
    if previous_data:
        with open(previous_data, 'rb') as file:
            (all_times, potential_energies, kinetic_energies, thermal_energies, 
                ...
                giant_center_of_mass_velocity, ms1_velocity, ms2_velocity, giant_mass, body1_mass, body2_mass, escaped_mass) = pickle.load(file)
    ...
    for i_step, time in enumerate(times):
        ...
        if len(computational_domain_removed_particles_temp) > 0:
            tot_removed_mass = computational_domain_removed_particles.total_mass()
            ...
            dM_removed = computational_domain_removed_particles.total_mass() - tot_removed_mass
        ...
        RL = 1.01 * estimate_escaping_radius(giant_system, sink_particles)
        ...
        if i_step % 2 == 0:
            ...
            with open(datafile, 'wb') as outfile:
                pickle.dump((all_times[:len(giant_center_of_mass)], potential_energies, 
                    ...
                    ms1_velocity, ms2_velocity, giant_mass, body1_mass, body2_mass, escaped_mass), outfile)
    ...
    mass_plot_combined((body1_mass-body1_mass[0]).as_quantity_in(units.MSun),(body2_mass-body2_mass[0]).as_quantity_in(units.MSun), \
...
def mass_plot_combined( b1_mass, b2_mass, semi_major_out, time, par_symbol, par_name="mass"):

    figure = pyplot.figure(figsize = (10, 6), dpi = 100)
    subplot1 = figure.add_subplot(2, 1, 1)
    plot(time,b1_mass,label='binary_prim')
    plot(time,b2_mass,label='binary_second')
    subplot1.set(xlabel=None)
    ylabel('Accreted' + '$'+par_symbol+'$')
    subplot1.legend()
...
if __name__ == "__main__":
    ...
    period_orbit_1 = 1.1 | units.day
    period_orbit_2 = 52.04 | units.day
    eccentricity_orbit_1 = 0.0
    eccentricity_orbit_2 = 0.3
    mutual_incliantion = 16.8
