...
def set_up_inner_binary():
    ...
    '''
    #semimajor_axis = 0.11879457854522224 | units.AU  #   0.133256133158 
    eccentricity = 0
    masses = [3.2, 3.1] | units.MSun 
    orbital_period = 7.15 | units.day
    semimajor_axis = orbital_period_to_semimajor_axis(orbital_period,masses[0],masses[1]).in_(units.au)
...
def set_up_outer_star(inner_binary_mass):
    ...
    '''
    #semimajor_axis = 1.0646508487341848 | units.AU  # from period :1.09491796572 au>, XiTau old:  1.22726535008
    eccentricity = 0.15     
    inclination_deg = 0.0
    inclination = math.radians(inclination_deg)  
    orbital_period = 145.5 | units.day 

    print("   Initializing outer star")
    giant = Particle()
    giant.mass = 3.98 | units.MSun # 16
    semimajor_axis = orbital_period_to_semimajor_axis(orbital_period, inner_binary_mass, giant.mass).in_(units.au)
    ...
    return giant
    print("   Initializing outer binary")
    print("   Orbital period outer binary:", orbital_period)
...
def triple_set_up_info(triple, view_on_giant, view_on_giant_se):
    ...
    t_koz = kozai_cycle_timescale(145.5| units.day ,7.15 | units.day ,triple[0].mass.value_in(units.MSun),
                      triple[1].mass.value_in(units.MSun),triple[2].mass.value_in(units.MSun),0.0).value_in(units.yr)
    lines.append("\nThe Lidov-Kozai  timescale is {:2f} yr".format( \
                        t_koz))
    
    oct_term = octupole_term(triple[0].mass.value_in(units.MSun),triple[1].mass.value_in(units.MSun),a_bin,a_giant,0.0)
...
def evolve_stars(triple, view_on_giant, stellar_code,radius_factor):
    stellar_code.parameters.AGB_wind_scheme = 0
    stellar_code.parameters.RGB_wind_scheme = 0
...
def relax_in_isolation(giant_in_sph, sph_code, enc_mass_prof, mult_factor, epsilon, where_to_save):
    ...
    n_steps = 100
    smoothing_length = epsilon*total_radius
    ...
    hydrodynamics.parameters.epsilon_squared = smoothing_length**2  # Softening removes the singularity in the inverse-square force
    ...
    energy_evolution_plot(times, kinetic_energies, potential_energies, thermal_energies,  
                    figname = where_to_save + "/energy_evolution_eps{:.4f}.png".format(epsilon))
...
'''
The parameter that needs to be optimized for a good relaxation of the stars is the softening length of the target core. The optimum softening length
...
to resolve with a smaller number of particles. Furthermore, the number of particles effects slightly the optimum softening length, thus it is preferable to start 
...
target_core_m_factor = numpy.arange(0.2,0.4,0.05)
   
epsilon_m_factor = numpy.array(list([0.2,0.25,0.3,0.35,0.4]))
...
for dummy_var, core_mult_f in enumerate(target_core_m_factor):
    ...
    triple_set_up_info(triple, view_on_giant,view_on_se_giant)
    path_to_dir_for_data = path+'/'+dir_name
    ...
    for eps in epsilon_m_factor:
        ...
        print("Relaxing giant with {:}".format(sph_code.__name__) + ' epsilon = {:.4f}'.format(eps))
