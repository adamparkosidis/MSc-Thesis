...
def evolve_coupled_system(binary_system, giant_system, giant_model, inner_binary, computational_domain_removed_particles, \
    ...
    if previous_data:
        ...
        RL = 1.0 * estimate_escaping_radius(giant_system, inner_binary)
    ...
    # Create sink particles based on the inner binary stars
    sink_cand = inner_binary.copy()
    sink_particles = new_sink_particles(sink_cand)
    
    # Now the sink radius is 4 larger than the initial stellar radius, it is close to the Roche lobe radius
    sink_particles.sink_radius= 4 * sink_cand.radius

    # Update the accretion radii of the sinks based on the Roche lobes,
    # which are calculated based on the inner binary's configuration
    ...
    # update the code gas particles based on the local gas particles
    ...
    for i_step, time in enumerate(times):
        ...
        RL = 1.0 * estimate_escaping_radius(giant_system, sink_particles)
